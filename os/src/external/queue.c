...
void queue_new(struct queue *q, uint64_t dsize) {
...
void queue_push(struct queue *q, void *d) {
  if (q->size == q->capacity) {
    ...
    memcpy(t + (q->capacity - (q->size - q->front)) * q->dsize,
           q->buffer + q->front * q->dsize, (q->size - q->front) * q->dsize);
...
int queue_empty(struct queue *q) { return !q->size; }

void queue_free(struct queue *q) { bd_free(q->buffer); }
